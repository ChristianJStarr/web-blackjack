...
class TestGameDbAdapter:
    ...
    def test_save_game_not_found(self, app):
        """Test saving a game that doesn't exist in the database."""
        # Setup
        game = BlackjackGame('abcd')
        
        with app.app_context():
            with patch('blackjack.game.adapters.Game') as mock_game_model:
                mock_game_model.query.get.return_value = None
                
                with patch('blackjack.game.adapters.db.session'):
                    mock_logger = MagicMock()
                    
                    with patch('blackjack.game.adapters.current_app.logger', mock_logger):
                        # Test
                        result = GameDbAdapter.save_game(game)
    ...
    @pytest.mark.usefixtures('app')
    def test_load_game_not_found_creates_new(self, app):
        """Test loading a game that doesn't exist creates a new one."""
        with app.app_context():
            with patch('blackjack.game.adapters.Game') as mock_game_model:
                mock_game_model.query.get.return_value = None
                
                with patch('blackjack.game.adapters.db.session') as mock_db_session:
                    mock_logger = MagicMock()
