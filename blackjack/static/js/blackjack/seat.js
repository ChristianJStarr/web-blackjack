...
export class Seat {
    constructor(table, seat_id) {
        ...
        this.join_seat_node = createElement('div', 'seat__join');
        ...
        this.join_seat_node.onclick = event => {
            if (this.table.game.user_id && !this.table.game.seat_id) {
                this.table.game.joinGame(this.id);
            } else if (!this.table.game.user_id) {
                window.modal.show('login');
            }
        }
    ...
    update(state) {
        try {
            ...
            const seat_id = this.table?.game?.seat_id ?? 0;
            ...
            if (!seat_id && !player?.id) {
                this.join_seat_node.style.display = 'flex';
                this.join_seat_node.textContent = 'Join Seat';
            }else {
                this.join_seat_node.style.display = 'none';
            }
    ...
    setPlayer(player) {
        if(player?.id) {
            this.player_node.style.display = 'block';
            this.player_node.textContent = player.name;
        }else {
            this.player_node.style.display = 'none';
        }
        this.player = player;
    }
    organizeChips() {
